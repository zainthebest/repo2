<div class="min-h-screen flex flex-col sm:justify-between items-center pt-6 sm:pt-0 bg-gray-100">


    <script>
        window.addEventListener('load', function () {
            setTimeout(function () {
                // window.location.reload(true);
                // TODO keep invoice status up-to-date
            }, 4000);
        });
    </script>
    <style>
        @media (min-width: 480px) {
            .invoice-area {
                max-width: 24rem;
            }
        }
    </style>

    <div class="w-full text-gray-600 invoice-area mt-6 px-6 py-4 bg-white shadow-md overflow-hidden sm:rounded-lg"
{{--         style="background: linear-gradient(45deg, #b082bf, #ff006a);--}}
{{--         color: #fff;"--}}
    >

        <h1 class="pb-3 text-2xl flex justify-between">
            <div>
                {{ $currency_sign }} {{ $invoice->amount }}
            </div>
            {{--            <span class="text-sm">&bullet;</span>--}}
            <div class="text-right @if($invoice->status == 'paid') text-green-600 @else text-red-600 @endif capitalize">
                @if($invoice->status == 'payment_detected')
                    Payment detected
                    <div class="text-sm text-gray-500 text-right">Waiting for payment confirmations</div>
                @else
                    {{ $invoice->status }}
                @endif

            </div>

            {{--            <span class="rounded-full b-8000 border-2 border-gray-400 text-sm p-3 px-5">5</span>--}}
        </h1>
        <hr>
        @if($invoice->status == 'paid')
            <div class="mt-5">
                <p class="text-gray-400">Payment has been received. Payment transaction
                    id {{ $invoice->payment()->id }}</p>
            {{--        TODO $invoice->store->return_url vs $store->return_url - donon mai kuch khas faraq to nhi --}}
            <!--<x-link href="{{ $invoice->store->success_url }}" class="mt-8 inline-block">Return to merchant site</x-link>-->
                <form action="{{ $invoice->store->success_url }}" method="POST">
                    <input type="hidden" name="transaction_id" value="{{ $invoice->payment()->id }}">
                    <input type="hidden" name="order_id" value="{{ $invoice->order_id }}">
                    <input type="hidden" name="invoice_amount" value="{{ $invoice->amount }}">
                    <input type="hidden" name="invoice_currency" value="{{ $invoice->currency_code }}">
                    <input type="hidden" name="invoice_status" value="paid">
                    <input type="submit"
                           class="mt-8 inline-block hover:underline text-blue-500 bg-transparent cursor:pointer"
                           value="Return to merchant site">
                    <inp
                </form>
            </div>
        @else
            <div class="mt-5">
                <span>Select a payment method</span>
                {{--            <p>--}}
                {{--                {{ dd($store->user_allowed_pay_gateways) }}--}}
                {{--            </p>--}}
                <div class="mt-4 max-h-64 overflow-auto" style="
                    display: grid;
                    grid-gap: 5px;
                    grid-template-columns: repeat(2, 1fr);">
                    <!-- Perfect money model -->
                    @if($store->acceptsPaymentVia('perfect_money'))
                        <x-jet-popup id="pm_model" maxWidth="md" bg="bg-gray-100 opacity-100">
                            <x-slot name="activator">
                                <button class="p-2 block w-full px-5 hover:bg-gray-100 font-bold" style="background: #eee;">
                                    {{ $perfect_money->display_name }}
                                </button>
                            </x-slot>

                            <div class="text-gray-400 p-2 font-semibold">
                                <button @click="show = false">< Back</button>
                            </div>

                            <!-- Modal contents -->
                            <div class="px-6 py-4">
                                <b class="text-lg">Pay with</b>
                                <hr>
                                <img class="w-3/4"
                                     src="https://www.pngitem.com/pimgs/m/579-5797921_perfect-money-logo-svg-hd-png-download.png"
                                >
                                <br>
                                <div class="text-xl">
                                    Total to pay:
                                    {{ $USD->symbol }} {{ $invoice->getAmountToBePaid('perfect_money') }}
                                </div>
                                <div class="text-xs text-gray-500">Fee
                                    of {{ $USD->symbol }} {{ $invoice->getFeeForPayer('perfect_money') }}</div>
                            </div>


                            <form action="https://perfectmoney.com/api/step1.asp" method="POST">
                                <input type="hidden" name="PAYEE_ACCOUNT" value="{{ $perfect_money->payments_to }}">
                                <input type="hidden" name="PAYMENT_AMOUNT"
                                       value="{{ $invoice->getAmountToBePaid('perfect_money') }}">
                                <input type="hidden" name="PAYMENT_UNITS" value="USD">
                                <input type="hidden" name="PAYEE_NAME"
                                       value="{{ config('app.name') }} - {{ $invoice->store->name }}">

                                <input type="hidden" name="PAYMENT_URL" value="{{ route('api.perfect_money.return') }}">
                                <input type="hidden" name="NOPAYMENT_URL"
                                       value="{{ route('api.perfect_money.return') }}">
                                <input type="hidden" name="STATUS_URL"
                                       value="{{ route('api.perfect_money.callback') }}">
                                <input type="hidden" name="PAYMENT_ID" value="{{ $invoice->token }}">
                                <input type="hidden" name="SUGGESTED_MEMO" value="{{ $inv_description }}">
                                <button
                                    class="ml-7 bg-blue-500 shadow-md text-white mt-10 mb-4 py-3 text-xl font-bold px-6 ">
                                    Pay now
                                </button>
                            </form>

                        </x-jet-popup>
                    @endif
                    @if(false)
                        <x-jet-popup id="bitcoin_model" maxWidth="md" bg="bg-gray-100 opacity-100">
                            <x-slot name="activator">
                                <button class="p-2 block w-full px-5 hover:bg-gray-100 font-bold" style="background: #eee;">
                                    {{ $blockchain->display_name }}
                                </button>
                            </x-slot>

                            <div class="text-gray-400 p-2 font-semibold">
                                <button @click="show = false">< Back</button>
                            </div>

                            <!-- Modal contents -->
                            <div class="px-6 py-4">
                                <b class="text-lg">Scan this Qr Code on your device</b>
                                <br>
                                <img class="mx-auto"
                                     src="https://chart.googleapis.com/chart?chs=250x250&cht=qr&chl={{ urlencode("bitcoin:$invoice->btc_address?amount=".$invoice->getAmountToBePaid('blockchain_btc')) }}&choe=UTF-8"
                                     alt="btc address qr code">
                                <x-jet-input type="text" readonly class="w-full"
                                             value="{{ $invoice->btc_address }}"></x-jet-input>
                                <div class="grid grid-flow-col grid-cols-2 text-md mt-3">
                                    <div>
                                        Total to pay:
                                    </div>
                                    <b class="text-lg">BTC {{ $invoice->getAmountToBePaid('blockchain_btc') }}</b>
                                </div>
                                <div class="text-md text-gray-500">Fee
                                    of {{ $invoice->getFeeForPayer('blockchain_btc') }} BTC apply
                                </div>
                            </div>


                            <a href="bitcoin:{{ $invoice->btc_address }}?amount={{ $invoice->getAmountToBePaid('blockchain_btc') }}"
                               class="ml-7 inline-block mx-auto bg-blue-500 shadow-md text-white mt-10 mb-4 py-3 text-xl font-bold px-6">
                                Pay with wallet
                            </a>

                        </x-jet-popup>
                    @endif
                    @if($store->acceptsPaymentVia('payeer'))
                        <x-jet-popup id="payeer_model" maxWidth="md" bg=" bg-gray-100 opacity-100">
                            <x-slot name="activator">
                                <button class="p-2 block w-full px-5 hover:bg-gray-100 font-bold" style="background: #eee;">
                                    {{ $payeer->display_name }}
                                </button>
                            </x-slot>

                            <div class="text-gray-400 p-2 font-semibold">
                                <button @click="show = false">< Back</button>
                            </div>

                            <!-- Modal contents -->
                            <div class="px-6 py-4">
                                <b class="text-lg">Pay with</b>
                                <hr>

                                <img class="w-2/4" src="https://makoli.com/wp-content/uploads/payeer-logo.png" alt="">
                                <br>
                                <div class="text-xl">Total to
                                    pay: {{ $USD->symbol }} {{ $invoice->getAmountToBePaid('payeer') }}</div>
                                <div class="text-xs text-gray-500">Fee
                                    of {{ $USD->symbol }} {{ $invoice->getFeeForPayer('payeer') }} added
                                </div>
                            </div>
                            @php

                                $m_shop = $payeer->payments_to; // merchant ID
                                $m_orderid = $invoice->token; // invoice number in the merchant's invoicing system
                                $m_amount = $invoice->getAmountToBePaid('payeer'); // invoice amount with two decimal places following a period
                                $m_curr = 'USD'; // invoice currency
                                $m_desc = base64_encode($invoice->prodcut_desc); // invoice description encoded using a base64
                                $m_key = $payeer->secret_key;
                                $arHash = array(
                                    $m_shop,
                                    $m_orderid,
                                    $m_amount,
                                    $m_curr,
                                    base64_encode($inv_description)
                                );

                                // Adding the secret key
                                $arHash[] = $m_key;
                                // Forming a signature
                                $sign = strtoupper(hash('sha256', implode(":", $arHash)));
                                //$sign = json_encode($arHash);
                            @endphp
                            <form method="post" action="https://payeer.com/merchant/">
                                <input type="hidden" name="m_shop" value="{{ $payeer->payments_to }}">
                                <input type="hidden" name="m_orderid" value="{{ $invoice->token }}">
                                <input type="hidden" name="m_amount" value="{{ $m_amount }}">
                                <input type="hidden" name="m_curr" value="USD">
                                <input type="hidden" name="m_desc" value="{{ base64_encode($inv_description) }}">
                                <input type="hidden" name="m_sign" value="{{ $sign }}">
                                <input type="submit"
                                       class="ml-7 bg-blue-500 shadow-md text-white mt-10 mb-4 py-3 text-xl font-bold px-6 "
                                       name="m_process" value="send"/>
                            </form>

                        </x-jet-popup>
                    @endif

                    @if($store->acceptsPaymentVia('alfalah'))
                        <x-jet-popup id="alfalah_model" maxWidth="md" bg=" bg-gray-100 opacity-100">
                            <x-slot name="activator">
                                <button class="p-2 block w-full px-5 hover:bg-gray-100 font-bold" style="background: #eee;">
                                    {{ $alfalah->display_name }}
                                </button>
                            </x-slot>

                            <div class="text-gray-400 p-2 font-semibold">
                                <button @click="show = false">< Back</button>
                            </div>

                            <!-- Modal contents -->
                            <div class="px-6 py-4">
                                <b class="text-lg">Pay with Debit Card</b>
                                <hr>
                                <br>

                                <div class="text-xl">Total to
                                    pay: {{ $PKR->symbol }} {{ $invoice->getAmountToBePaid('alfalah') }}</div>
                                <div class="text-xs text-gray-500">Fee
                                    of {{ $PKR->symbol }} {{ $invoice->getFeeForPayer('alfalah') }} added
                                </div>
                            </div>
                            <form action="https://payments.bankalfalah.com/HS/HS/HS" method="post">
                                <input type="hidden" name="HS_RequestHash" value="{{ $alfalah->request_hash }}">
                                <input type="hidden" name="HS_IsRedirectionRequest" value="1">
                                <input type="hidden" name="HS_ChannelId" value="1001">
                                <input type="hidden" name="HS_ReturnURL" value="{{ $alfalah->return_url }}">
                                <input type="hidden" name="HS_MerchantId" value="{{ $alfalah->payments_to }}">
                                <input type="hidden" name="HS_StoreId" value="{{ $alfalah->other_data['store_id'] }}">
                                <input type="hidden" name="HS_MerchantHash"
                                       value="{{ $alfalah->other_data['merchant_hash'] }}">
                                <input type="hidden" name="HS_MerchantUsername"
                                       value="{{ $alfalah->other_data['merchant_username'] }}">
                                <input type="hidden" name="HS_MerchantPassword"
                                       value="{{ $alfalah->other_data['merchant_password'] }}">
                                <input type="hidden" name="HS_TransactionReferenceNumber"
                                       value="{{ $alfalah->trx_ref_num }}">
                                <button type="submit"
                                        class="ml-7 bg-blue-500 shadow-md text-white mt-10 mb-4 py-3 text-xl font-bold px-6">
                                    Pay Now
                                </button>
                            </form>

                        </x-jet-popup>
                    @endif

                    @if($store->acceptsPaymentVia('internal_transfer'))
                        <x-jet-popup id="payViaAccount_model" maxWidth="md" bg=" bg-gray-100 opacity-100">
                            <x-slot name="activator">
                                <button class="p-2 block w-full px-5 hover:bg-gray-100 font-bold" style="background: #eee;">
                                    Pay via <i>UpzarPay</i>
                                </button>
                            </x-slot>

                            <div class="text-gray-400 p-2 font-semibold">
                                <button @click="show = false">< Back</button>
                            </div>

                            <!-- Modal contents -->
                            <div class="px-6 py-4">
                                <b class="text-lg">Pay with UpzarPay account balance</b>
                                <hr>
                                <br>

                                Continue to your UpzarPay account?
                            </div>

                            <form action="{{ route('invoice.pay_with_funds', $invoice->token) }}" method="get">
                                <button type="submit"
                                        class="ml-7 bg-blue-500 shadow-md text-white mt-10 mb-4 py-3 text-xl font-bold px-6">
                                    Continue to account
                                </button>
                            </form>

                        </x-jet-popup>
                    @endif

                    @if($store->acceptsPaymentVia('jazzcash'))
                        <x-jet-popup id="jazzcash_model" maxWidth="md" bg=" bg-gray-100 opacity-100">
                            <x-slot name="activator">
                                <button class="p-2 block w-full px-5 hover:bg-gray-100 font-bold" style="background: #eee;">
                                    {{ $jazzcash->display_name }}
                                </button>
                            </x-slot>

                            <div class="text-gray-400 p-2 font-semibold">
                                @if($jazzcash_screen == 1 || $jazzcash_screen == 3)
                                    <button @click="show = false">< Back</button>
                                @elseif($jazzcash_screen == 2)
                                    <button wire:click="$set('jazzcash_screen', 1)">< Back</button>
                                @endif
                            </div>


                            <!-- Modal contents -->
                            <div class="px-6 py-4">
                                <b class="text-lg">Pay with </b>
                                <img class="w-2/6 mt-1"
                                     src="data:image/png;base64,"
                                >
                                <br>
                                @if($jazzcash_screen == 1)
                                    <div class="text-xl">Total to
                                        pay: {{ $PKR->symbol }} {{ $invoice->getAmountToBePaid('jazzcash') }}</div>
                                    <div class="text-xs text-gray-500">Fee
                                        of {{ $PKR->symbol }} {{ $invoice->getFeeForPayer('jazzcash') }} added
                                    </div>
                                @endif
                            </div>
                            @if($jazzcash_screen == 1)
                                <form class="px-6 py-4 border-t-2" wire:submit.prevent="jazzcash_verify">
                                    <x-jet-label for="jc_phone_number" value="{{ __('Enter your phone number') }}"/>
                                    <x-jet-input id="jc_phone_number" type="number" class="block mt-1"
                                                 wire:model.defer="jc_phone_number"/>
                                    <x-jet-input-error for="jc_phone_number" class="mt-2"/>
                                    <button type="submit" wire:loading.attr="disabled"
                                            class="disabled:opacity-50 mt-1 bg-blue-500 shadow-md text-white mb-4 px-2 py-1 font-bold ">
                                        Send OTP
                                    </button>

                                </form>
                            @endif
                            @if($jazzcash_screen == 2)
                                <form class="px-6 py-4 border-t-2" wire:submit.prevent="jazzcash_confirm">
                                    <span class="mb-2 block text-gray-700">A verification code has been sent to: {{ $jc_phone_number }}. Enter that code below.</span>
                                    <x-jet-label for="jazzcash_code" value="{{ __('Enter verification code') }}"/>
                                    <x-jet-input id="jazzcash_code" type="number" class="block mt-1"
                                                 wire:model.defer="jazzcash_code"/>
                                    <x-jet-input-error for="jazzcash_code" class="mt-2"/>
                                    <button type="submit"
                                            class="disabled:opacity-50 mt-1 bg-blue-500 shadow-md text-white mb-4 px-2 py-1 font-bold ">
                                        Verify
                                    </button>

                                </form>
                            @endif
                            @if($jazzcash_screen == 3)
                                <form class="px-6 py-4 border-t-2" wire:submit.prevent="jazzcash_verify">
                                    <span class="mb-2 block text-red-600">
                                        Transaction failed!
                                    </span>
                                    <x-jet-label for="jc_phone_number" value="{{ __('Enter your phone number') }}"/>
                                    <x-jet-input id="jc_phone_number" type="number" class="block mt-1"
                                                 wire:model.defer="jc_phone_number"/>
                                    <x-jet-input-error for="ep_phone_number" class="mt-2"/>
                                    <button type="submit" wire:loading.attr="disabled"
                                            class="disabled:opacity-50 mt-1 bg-blue-500 shadow-md text-white mb-4 px-2 py-1 font-bold ">
                                        Send OTP
                                    </button>

                                </form>
                            @endif
                        </x-jet-popup>
                    @endif

                    @if($store->acceptsPaymentVia('paypal'))
                        <x-jet-popup id="paypal_model" maxWidth="md" bg=" bg-gray-100 opacity-100">
                            <x-slot name="activator">
                                <button class="p-2 block w-full px-5 hover:bg-gray-100 font-bold" style="background: #eee;">
                                    {{ $paypal->display_name }}
                                </button>
                            </x-slot>

                            <div class="text-gray-400 p-2 font-semibold">
                                <button @click="show = false">< Back</button>
                            </div>

                            <!-- Modal contents -->
                            <div class="px-6 py-4">
                                <b class="text-lg">Pay with </b>
                                <hr>
                                <img class="w-2/5 mt-1"
                                     src="https://www.paypalobjects.com/images/shared/paypal-logo-129x32.svg"
                                >
                                <br>

                                <div class="text-xl">Total to
                                    pay: {{ $USD->symbol }} {{ $invoice->getAmountToBePaid('paypal') }}</div>
                                <div class="text-xs text-gray-500">Fee
                                    of {{ $USD->symbol }} {{ $invoice->getFeeForPayer('paypal') }} added
                                </div>
                            </div>

                            <form action="https://www.sandbox.paypal.com/cgi-bin/webscr" method="post">
                                <!-- Identify your business so that you can collect the payments. -->
                                <input type="hidden" name="business" value="{{ $paypal->payments_to }}">
                                <!-- Specify a Buy Now button. -->
                                <input type="hidden" name="cmd" value="_xclick">
                                <!-- Specify details about the item that buyers will purchase. -->
                                <input type="hidden" name="item_name" value="{{ $inv_description }}">
                                <input type="hidden" name="amount" value="{{ $invoice->getAmountToBePaid('paypal') }}">
                                <input type="hidden" name="currency_code" value="USD">

                                <input type="hidden" name="notify_url" value="{{ route('api.paypal.callback') }}">
                                <input type="hidden" name="return" value="{{ route('invoice.show', $invoice->token) }}">

                                <!-- Define returning variables -->
                                <input type="hidden" name="invoice" value="{{ $invoice->token }}">

                                <!-- Display the payment button. -->
                                <button type="submit"
                                        class="ml-7 bg-blue-500 shadow-md text-white mt-10 mb-4 py-3 text-xl font-bold px-6">
                                    Pay Now
                                </button>
                            </form>
                        </x-jet-popup>

                    @endif

                    @if($store->acceptsPaymentVia('easypaisa'))
                        <x-jet-popup id="easypaisa_model" maxWidth="md" bg=" bg-gray-100 opacity-100">
                            <x-slot name="activator">
                                <button class="p-2 block w-full px-5 hover:bg-gray-100 font-bold" style="background: #eee;">
                                    Easypaisa
                                </button>
                            </x-slot>

                            <div class="text-gray-400 p-2 font-semibold">
                                @if($easypaisa_screen == 1 || $easypaisa_screen == 3)
                                    <button @click="show = false">< Back</button>
                                @elseif($easypaisa_screen == 2)
                                    <button wire:click="$set('easypaisa_screen', 1)">< Back</button>
                                @endif
                            </div>


                            <!-- Modal contents -->
                            <div class="px-6 py-4">
                                <b class="text-lg">Pay with </b>
                                <img class="w-2/5 mt-1"
                                     src="https://easypaisa.com.pk/wp-content/uploads/2019/10/Header-Icon.png"
                                >

                                <br>
                                @if($easypaisa_screen == 1)
                                    <div class="text-xl">Total to
                                        pay: {{ $PKR->symbol }} {{ $invoice->getAmountToBePaid('easypaisa') }}</div>
                                    <div class="text-xs text-gray-500">Fee
                                        of {{ $PKR->symbol }} {{ $invoice->getFeeForPayer('easypaisa') }} added
                                    </div>
                                @endif
                            </div>
                            @if($easypaisa_screen == 1)
                                <form class="px-6 py-4 border-t-2" wire:submit.prevent="easypaisa_verify">
                                    <x-jet-label for="ep_phone_number" value="{{ __('Enter your phone number') }}"/>
                                    <x-jet-input id="ep_phone_number" type="number" class="block mt-1"
                                                 wire:model.defer="ep_phone_number"/>
                                    <x-jet-input-error for="ep_phone_number" class="mt-2"/>
                                    <button type="submit" wire:loading.attr="disabled"
                                            class="disabled:opacity-50 mt-1 bg-blue-500 shadow-md text-white mb-4 px-2 py-1 font-bold ">
                                        Send OTP
                                    </button>

                                </form>
                            @endif
                            @if($easypaisa_screen == 2)
                                <form class="px-6 py-4 border-t-2" wire:submit.prevent="easypaisa_confirm">
                                    <span class="mb-2 block text-gray-700">A verification code has been sent to: {{ $ep_phone_number }}. Enter that code below.</span>
                                    <x-jet-label for="easypaisa_code" value="{{ __('Enter verification code') }}"/>
                                    <x-jet-input id="easypaisa_code" type="number" class="block mt-1"
                                                 wire:model.defer="easypaisa_code"/>
                                    <x-jet-input-error for="easypaisa_code" class="mt-2"/>
                                    <button type="submit"
                                            class="disabled:opacity-50 mt-1 bg-blue-500 shadow-md text-white mb-4 px-2 py-1 font-bold ">
                                        Verify
                                    </button>

                                </form>
                            @endif
                            @if($easypaisa_screen == 3)
                                <form class="px-6 py-4 border-t-2" wire:submit.prevent="easypaisa_verify">
                                    <span class="mb-2 block text-red-600">
                                        Transaction failed!
                                    </span>
                                    <x-jet-label for="ep_phone_number" value="{{ __('Enter your phone number') }}"/>
                                    <x-jet-input id="ep_phone_number" type="number" class="block mt-1"
                                                 wire:model.defer="ep_phone_number"/>
                                    <x-jet-input-error for="ep_phone_number" class="mt-2"/>
                                    <button type="submit" wire:loading.attr="disabled"
                                            class="disabled:opacity-50 mt-1 bg-blue-500 shadow-md text-white mb-4 px-2 py-1 font-bold ">
                                        Send OTP
                                    </button>

                                </form>
                            @endif
                        </x-jet-popup>
                    @endif

                </div>


            </div>
        @endif
    </div>


    <footer class="text-gray-500 mb-5">
        Powered by <a href="{{ route('landing_page') }}" class="font-bold" target="_blank">UpzarPay</a>
    </footer>

</div>
